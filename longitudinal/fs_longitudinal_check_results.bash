...
for subject in $subjects; do
  ...
  if [ "$subject_is_missing_input" = "yes" ]; then
    num_missing_input_so_far=$(echo $subjects_missing_input | wc -w | tr -d '[:space:]') # so far, before the current one.
    if [ $num_missing_input_so_far -gt 0 ]; then # avoid extra space for first subject so we can properly split at the end. should use array instead.
      subjects_missing_input="${subjects_missing_input} ${subject}"
    else
      subjects_missing_input="${subject}" # no space here
    fi
  fi
