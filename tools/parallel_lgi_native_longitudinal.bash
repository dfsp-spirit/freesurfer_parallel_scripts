...
# 3) path/to/parallel_lgi_native_longitudinal.bash ./subjects.txt
#
# Note that this script assumes that the subjects.txt file contains the long names for the subjects (like 'subject1_MR1.long.subjct1')
...
for SUBJECT in $SUBJECTS; do
  ...
  if [ ! -d "${SUBJECTS_DIR}/${BASE_SUBJECT}" ]; then
    echo "$APPTAG ERROR: Directory for base subject '${BASE_SUBJECT}' not found in SUBJECTS_DIR '${SUBJECTS_DIR}'. Exiting."
    exit 1
  fi
done
